...
public class MortarMenu extends AbstractContainerMenu {
    ...
    public void tick() {
        if (this.player.level().isClientSide()) return; // Só o servidor calcula matemática
        ...
        if (recipeOpt.isPresent()) {
            ...
            // 1. ANTES DE PROCESSAR: Verifica se há espaço nos slots de saída (6 ou 7) para o resultado
            if (!canFitResult(recipe)) {
                // Se os slots de saída estão cheios, segura o progresso e não deixa travar rodando
                return;
            }

            this.progress++;

            // Sincroniza a barrinha com a tela do cliente
    ...
    private boolean canFitResult(MortarRecipe recipe) {
        // Testa se pelo menos o primeiro item do resultado cabe nos slots de saída (6 ou 7)
        for (ChanceResult chanceResult : recipe.results()) {
            ...
            if (!fits) return false; // Sem espaço, trava a máquina
        }
        return true;
    }

    private void craftItem(MortarRecipe recipe) {
        // 1. Consome exatamente a quantidade de pó de qualquer um dos 6 slots
        ...
        for (ChanceResult chanceResult : recipe.results()) {
            // Se a chance for menor que 1.0, roleta o dado (ex: random.nextFloat() < 0.5 para 50%)
            if (chanceResult.chance() < 1.0f && this.player.getRandom().nextFloat() >= chanceResult.chance()) {
                continue; // Falhou no teste de sorte, pula este item!
            }
        ...
        // 2. O Som da química acontecendo!
        this.player.level().playSound(null, this.player.blockPosition(), SoundEvents.GRINDSTONE_USE, SoundSource.BLOCKS, 1.0F, 0.7F);

        // 3. Joga o resultado nos slots testando a chance
        for (ChanceResult chanceResult : recipe.results()) {
            // Se a chance for menor que 1.0, roleta o dado
            if (chanceResult.chance() < 1.0f && this.player.getRandom().nextFloat() >= chanceResult.chance()) {
                continue; // Falhou no teste de sorte, pula este item!
            }

            // Pegamos o ItemStack correto de dentro do ChanceResult
            ItemStack outputCopy = chanceResult.stack().copy();
